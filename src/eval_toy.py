...
CONFIG = {
    ...
    "max_new_tokens": 16,
    "num_beams": 1,
    "model_type": "prefix" # prefix or encoder-decoder
}

# model_path = "./models/vit-gpt2-coco-en_2026-01-26--11-18-56",
model_path = "./models/clip-vit-base-patch32_mGPT_2026-01-27--19-39-56" # prefix model
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader, desc="eval+save")):
        ...
        if batch_idx == 0:
            ...
            if CONFIG["model_type"] != "prefix":
                out = model.decoder(input_ids=dec_ids, encoder_hidden_states=enc.last_hidden_state)
            else:
                out = model.decoder(input_idx=dec_ids) # don't pass hidden states to the prefix model
...
def main():
    ...
    try:
        ...
        if CONFIG['model_type'] == 'encoder-decoder':
            ...
            ONFIG["data_path"] = "./data/toy_colors_vit-gpt2-coco-en.pt"
            ...
            CONFIG["data_path"] = "./data/toy_colors_clip-vit-base-patch32.pt"
